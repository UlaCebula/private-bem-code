...
for i=1:length(prop.r_R)-1
    ...
    prop.sectionpitch = pitch_distribution(prop.sectionchord,prop.collective_blade_twist, propellertype);% [deg]
%     if (DEBUG)
%          = SolveSection(i, polar, prop, air, oper);
%     else
%     end
%   [alpha, a, aprime, f_axial, f_tangential, gamma, iterationcount]
...
function aprime = AzimInductionFactor(Fazim, prop, air, SectionRadius, oper, a)    
    top = Fazim * prop.Nblades;
    btm = 2*pi*oper.U_inf*(1-a)*oper.omega*2*SectionRadius^2;    
    aprime = top/btm;
end
...
function [f_axial, f_tangential, gamma, alpha, cl, cd, InflowAngle] = SectionLoads(U_axial, U_tangential, prop, polar, air)
...
function Results = SolveSection(index, polar, prop, air, oper)
    ...
    for i=1:N
        ...
        [f_axial, f_tangential, gamma, alpha, cl, cd, ~]=SectionLoads(U_axial, U_tangential,prop, polar, air);
        ...
        aprime = AzimInductionFactor(f_tangential, prop, air, SectionRadius, oper, a);
    ...
    Results = [SectionRadius/prop.R, a, aprime, alpha, cl, cd,SectionArea, i, Prandtl];
end
