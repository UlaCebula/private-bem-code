...
hold on
plot(radialdist_zeroyaw, a_zeroyaw,'b');
...
%     if (CT>0.96*F)%(CT, F, sigma, cx, phi)
...
function aprime = AzimInductionFactor(Fazim, prop, SectionRadius, oper, a, air)
    top = Fazim * prop.Nblades;%(Fazim, prop, SectionRadius, oper, a, air)   
    btm = 2*air.density*pi*oper.U_inf*(1-a)*oper.omega*2*SectionRadius^2;    
    aprime = top/btm;
    
    %%%%%%%%%%%
%     top = 4.*F.*sin(phi).*cos(phi);(F, phi, sigma, cy)  
%     btm = sigma.*cy;
%     aprime = (-1+(top./btm)).^-1;
...
%     F1 = (-prop.Nblades/2)*((1-r_R_Section)/r_R_Section)*(1/abs(sin(InflowAngle)));(prop, SectionRadius, InflowAngle
%     F2 = (-prop.Nblades/2)*((r_R_Section-prop.blade_root)/prop.blade_root)*(1/abs(sin(InflowAngle)));
...
function [f_axial, f_tangential, gamma, alpha, U_resultant, cx,cy] = SectionLoads(U_axial, U_tangential, prop, polar, air)
...
%         a_new = AxialInductionFactor(CT_streamtube, Prandtl, SectionRotorSolidity, cx, InflowAngle);
%         a = 0.75*a+0.25*a_new;%update new value of axial induction via weighted average
%         aprime = AzimInductionFactor(Prandtl, InflowAngle, SectionRotorSolidity, cy); 
%         
    ...
    Results = [SectionRadius/prop.R, a, aprime, f_axial, f_tangential,gamma, InflowAngle,  i];
end
