...
for i=1:length(prop.r_R)-1
    for j=1%for each angle
        ...
        [SectionResults(i,:), a(i,j), aprime(i,j), alpha(i,j), inflowangle(i,j)] = SolveSection(i, polar, prop, air, oper, AzimuthAngle(j), dPsi, yaw);
    end
end
...
%     SectionArea = pi*(((r_R2*prop.R)^2)-((r_R1*prop.R)^2)); %[m^2]
    ...
    for i=1:N
        ...
        [inflowangle, alpha, cx, cy] = SectionInflowAngleCalc(oper, a, aprime, SectionRadius, polar, prop);
...
function [phi, alpha, cx, cy] = SectionInflowAngleCalc(oper, a, aprime, SectionRadius, polar, prop)
    U_axial = oper.U_inf*(1-a);
    U_tan = oper.omega*SectionRadius*(1+aprime);
...
function a_skew = SkewedWakeCorrection(a,SectionRadius,prop,chi,AzimuthAngle)
    ...
    temp = (15*pi/32).*r_R.*tan(chi/2).*cos(AzimuthAngle);
    a_skew = a.*(1+temp);
end